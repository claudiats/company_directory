<?php
	...
	$statement = 'SELECT * from personnel WHERE ' . $field . ' LIKE ?';

	if(!empty($depts)){
	 	$statement = 'SELECT * from personnel WHERE (' . $field . ' LIKE ?) AND find_in_set(departmentID, ?)';
	}
    ...
    if(empty($depts)){
		...
		$query->bind_param("ss", $term, $depts);
    }
